...
pub fn do_handle_transact(state: state.DbState, facts: List(fact.Fact), valid_time: Option(Int), op: fact.Operation, reply: process.Subject(Result(state.DbState, String)), compute_next_state: fn(state.DbState, List(fact.Fact), Option(Int), fact.Operation) -> Result(#(state.DbState, List(fact.Datom)), String)) -> actor.Next(state.DbState, msg) {
  case compute_next_state(state, facts, valid_time, op) {
    Ok(#(final_state, datoms)) -> {
      ...
      process.send(state.reactive_actor, state.Notify(changed_attrs, final_state))
